...
contract SimpleBoardTest is MudTest {
  ...
  function setUp() public override {
    ...
    words.push(keccak256(abi.encodePacked([Letter.H, Letter.E, Letter.L, Letter.L, Letter.O]))); // hello
    words.push(keccak256(abi.encodePacked([Letter.W, Letter.O, Letter.R, Letter.D]))); // word
    words.push(keccak256(abi.encodePacked([Letter.R, Letter.O, Letter.A, Letter.D]))); // road
  }

  function testSetup() public prank(deployer) {
  ...
  function testPlayTwoWords() public prank(deployer) {
    vm.deal(deployer, 2 ether);
    ...
    Tile memory secondTile = tiles.getValueAtPosition(Position({ x: 7, y: 1 }));
    assertTrue(secondTile.letter == Letter.L);
    assertTrue(tiles.hasTileAtPosition(Position({ x: 8, y: -1 })));
    Tile memory rightTile = tiles.getValueAtPosition(Position({ x: 8, y: -1 }));
    ...
    LetterCountComponent letterCount = LetterCountComponent(component(LetterCountComponentID));
    ...
    // Verify treasury
    assertTrue(boardSystem.getTreasury() == (uint256(1 ether) * 3) / 4 + (uint256(1 ether) * 3) / 4);
  }

  function testCannotPlayOverWord() public prank(deployer) {
    vm.deal(deployer, 2 ether);
    BoardSystem boardSystem = BoardSystem(system(BoardSystemID));
    boardSystem.setMerkleRoot(m.getRoot(words));
    boardSystem.setupInitialGrid();
  ...
  function testCannotPlayEmptyWord() public prank(deployer) {
    vm.deal(deployer, 2 ether);
    BoardSystem boardSystem = BoardSystem(system(BoardSystemID));
    boardSystem.setMerkleRoot(m.getRoot(words));
    boardSystem.setupInitialGrid();
  ...
  function testCannotImmediatelyClaimRewards() public prank(deployer) {
    vm.deal(deployer, 2 ether);
    BoardSystem boardSystem = BoardSystem(system(BoardSystemID));
    boardSystem.setMerkleRoot(m.getRoot(words));
    boardSystem.setupInitialGrid();
  ...
  function testCannotPlayWordsNotConnect() public prank(deployer) {
    vm.deal(deployer, 2 ether);
    BoardSystem boardSystem = BoardSystem(system(BoardSystemID));
    boardSystem.setMerkleRoot(m.getRoot(words));
    boardSystem.setupInitialGrid();
